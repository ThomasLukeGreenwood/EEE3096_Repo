...
int main(void)
{
  ...
  while (1)
  {
	...
	pollADC(); //update print_val to last ADC value
	writeLCD(print_val); //send print_val to LCD


	// Update PWM value; TODO: Get CRR
	CCR = ADCtoCCR(adc_val); //update CRR to converted adc_val
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	...
	HAL_ADC_Stop(&hadc);
	sprintf(print_val, "%d", adc_val); //convert adc_val to a string and save to print_val
}
