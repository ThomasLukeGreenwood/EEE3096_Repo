...
//defines all the LED patterns that will be displayed
static uint8_t patterns[] =   {0b10101010, 0b01010101,0b11001100,0b00110011,0b11110000,0b00001111};

//EEPROM Memory address we will use for the storage of the pattern in hex for each reading
static uint16_t memAdrress[] = {0x0,0x1,0x2,0x3,0x4,0x5};

//Current index of the memory location
uint16_t current_address = 0; // need to have a start values for the address.

//To switch between 1s interrupts and 0.5s interrupts
uint8_t toggle =0;

//for debounce protection of the buttons
uint8_t buttonState;
...
int main(void)
{
  ...
  //Starts the interrupt based timer
  HAL_TIM_Base_Start_IT(&htim16);

  // TODO: Write all "patterns" to EEPROM using SPI

  //writes all the patterns to the EEPROM
  for(int i = 0; i < sizeof(patterns); i++)
  {
	  write_to_address(memAdrress[i],patterns[i]);
  }
  /* USER CODE END 2 */
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	 ...
	 // This will use a toggle to switch between 0.5 s and 1s and has debounce protection

	 buttonState = HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin);
	 if((HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin)) != buttonState)
	 {
		 toggle = !toggle;
		 if(toggle)
		 {
			...
			/*To change the prescaler instead of the ARR
			 htim16.Init.Prescaler = (8000/2)-1;
		 ...
		 else
		 {
			 ...
			 /*To change the pre-scaler instead of the ARR
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t data = read_from_address(memAdrress[current_address]);

	//fail safe code TASK5
	if (data != patterns[current_address])
	{
		//if the EEPROM doesn't read correct data send error code
		GPIOB -> ODR = 0x1;
	}
	...
	//checks if the current address is within the index of all the data stored, if not reset to start
	if(current_address < sizeof(patterns)-1)
	{
